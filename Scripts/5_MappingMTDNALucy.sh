...
mkdir -p ${BASE_PATH}/Data/4_MappingMTDNA/mapped
mkdir -p ${BASE_PATH}/Data/4_MappingMTDNA/qualimap

#specify paths (Note: you may need to change PATH_TRIMMED if you needed to run additional trimming to remove all adapters)
PATH_TRIMMED="${BASE_PATH}/Data/2_AdapterRemoval/AdapterROutput"
PATH_MAP="${BASE_PATH}/Data/4_MappingMTDNA/mapped"
PATH_QUALI="${BASE_PATH}/Data/4_MappingMTDNA/qualimap"
PATH_REF=/dartfs-hpc/rc/home/5/f008715/FleskesR/ReferenceSeqs/MtDNA
...
for j in *truncated.gz
do   
    NAME=${j%.fastq.truncated.gz}  #Change ".fastq.truncated.gz" to correct file suffix if you had to run other trimming programs
    echo "Running..."${NAME}""
